  ...
  return (
    <main className="min-h-screen bg-[#F4F5F7]">
      ...
      <div className="relative w-full h-[50vh] min-h-[320px] overflow-hidden">
        {receta.imageUrl ? (
          <Image src={receta.imageUrl} alt={receta.title} fill className="object-cover object-top" priority />
        ) : (
          ...
          <div className="bg-white rounded-3xl overflow-hidden shadow-sm">
            ...
            <div className="divide-y divide-gray-50">
              {receta.ingredients.map((ing: any, i: number) => (
                <div key={i} className="px-7 py-4 flex gap-4 items-baseline">
                  <span className="inline-block bg-[#1E5631] text-white text-[10px] font-bold tracking-wider uppercase px-2.5 py-1 rounded-lg whitespace-nowrap shrink-0">
                    {ing.name}
                  </span>
        ...
        {receta.steps?.length > 0 && (
          <div className="bg-white rounded-3xl overflow-hidden shadow-sm">
            ...
            <div className="p-7 space-y-6">
              {receta.steps.flatMap((step: string) => {
                // Si el paso contiene numeración "1. ... 2. ..." lo parseamos
                const numbered = step.match(/\d+\.\s[^0-9]+(?=\d+\.|$)/g);
                if (numbered && numbered.length > 1) {
                  return numbered.map((s: string) => s.trim());
                }
                return [step];
              }).map((step: string, i: number) => {
                // Limpiar el "1. " del inicio si viene del texto
                const clean = step.replace(/^\d+\.\s*/, "").trim();
                const title = clean.includes(":") ? clean.split(":")[0] : null;
                const body = title ? clean.split(":").slice(1).join(":").trim() : clean;
                return (
                  <div key={i} className="flex gap-5">
                    <div className="w-9 h-9 bg-[#FF6B35] text-white text-sm font-black rounded-full flex items-center justify-center shrink-0 mt-0.5">
                      {i + 1}
                    </div>
                    <div className="flex-1 pt-1">
                      {title && (
                        <p className="font-bold text-[#2B2D42] text-sm mb-1">{title}</p>
                      )}
